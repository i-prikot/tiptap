...
- [x] **Task 1: Replace obsolete embedding guidance with a local-integration entry point.**
  - [ ] Files: `README.md`.
...
- [x] **Task 2: Add an end-to-end `npm pack` workflow for installing local tarballs in the Tinyfy cabinet.**
  - [ ] Files: `README.md`.
  ...
  - [ ] Document package order and explicit commands for `@i-prikot/editor-schema` before `@i-prikot/editor`; include `@i-prikot/editor-renderer` only as an optional tarball when the cabinet uses static rendering.
...
- [x] **Task 3: Add a Vite alias alternative for live development against workspace sources.**
...
- [x] `README.md` gives a Tinyfy cabinet developer one discoverable local-integration section with a clear choice between tarballs and source aliases.
- [x] The tarball instructions build, pack, and install the editor schema before the editor, with an optional renderer path and an explicit refresh loop.
- [x] The alias instructions exactly cover the editor entry, schema entry, stylesheet subpath, and Vite filesystem access needed by a separate local checkout.
- [x] The guide clearly distinguishes source-alias iteration from packed-artifact validation and calls out peer-runtime compatibility risks.
- [x] No application code, package metadata, configuration, tests, lockfiles, or roadmap artifacts are changed.
