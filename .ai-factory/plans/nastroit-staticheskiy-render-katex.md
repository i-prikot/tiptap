...
- [x] Make KaTeX CSS a deliberate package contract: expose `@i-prikot/renderer/katex.css`, whose source imports `katex/dist/katex.css`. The consuming public-page build must include this CSS asset so KaTeX fonts and layout are served by the stylesheet pipeline; no JavaScript runtime import is permitted.
