...
- [x] Keep `@i-prikot/editor-schema` free of `vue`, `@tiptap/vue-3`, Vue SFCs, and `VueNodeViewRenderer`.
...
- [x] **1. Define root workspace orchestration and shared TypeScript/tooling configuration.**
  ...
  - [x] Add npm `workspaces` for `packages/*` and `apps/*`; keep the root `private`; replace single-app scripts with root orchestration commands that delegate to `@i-prikot/playground` and run workspace-aware build, typecheck, lint, formatting, and hook workflows.
...
- [x] **2. Extract an isomorphic `@i-prikot/editor-schema` package.**
...
- [x] **3. Extract the Vue-facing `@i-prikot/editor` package and restore NodeViews.**
  ...
  - [x] Implement editor-owned adapters that extend the schema's image, image-upload, and TOC node definitions with `VueNodeViewRenderer`, then assemble the interactive editor extension list from `@i-prikot/editor-schema` plus these adapters.
  - [x] Update all moved imports: editor-to-schema imports use `@i-prikot/editor-schema`; editor internals use relative paths; remove root alias dependencies and preserve exported components/types currently exposed by `src/editor/index.ts`.
  ...
  - [x] Declare `@i-prikot/editor-schema` with the npm-compatible matching local version `0.1.0` (npm creates the workspace link) and assign Vue, `@tiptap/vue-3`, Floating UI, collaboration client, and other editor-only runtime dependencies to this package.
  - [x] **Logging:** no runtime logs; add no debug statements while relocating UI code, and make package typecheck failures identify `@i-prikot/editor`.
  - [x] **Depends on:** task 2.

### Phase 2 — Move the host application and complete all workspaces

- [x] **4. Move the existing demo into `@i-prikot/playground` and wire it to package APIs.**
  ...
  - [x] Replace direct imports from former `src/editor/**` paths with the public exports of `@i-prikot/editor`, import the editor stylesheet through its package export, and keep app-specific document ID, seed content, environment reads, collaboration setup, theme UI, and CTA behavior inside the playground.
  ...
  - [x] Declare `@i-prikot/editor` and `@i-prikot/editor-schema` with npm-compatible matching local versions `0.1.0` (npm creates workspace links); leave application-specific dev/runtime configuration in the playground manifest.
...
- [x] **6. Finalize dependency ownership, lockfile state, and repository-wide tooling resolution.**
  ...
  - [x] Validate the migration with `npm run typecheck`, `npm run lint`, `npm run build`, and a manual `npm run dev` smoke start for `@i-prikot/playground`. Do not run or add test work because testing is disabled for this task.
...
- [x] **Address blocking finding `02c6b3933c15` / `9a64e415a4da`: declare `@tiptap/extension-emoji` directly in `@i-prikot/editor`.**
  ...
  - [x] Verified with `npm run typecheck --workspace=@i-prikot/editor` and `npm ls --workspace=@i-prikot/editor @tiptap/extension-emoji --depth=0`.
