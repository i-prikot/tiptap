...
const passiveScrollListenerOptions: AddEventListenerOptions = { passive: true }
...
onMounted(() => {
  window.addEventListener('scroll', handleScroll, passiveScrollListenerOptions)
  scrollParent = rootRef.value ? getScrollableAncestor(rootRef.value) : null
  if (scrollParent && !(scrollParent instanceof Window)) {
    scrollParent.addEventListener('scroll', handleScroll, passiveScrollListenerOptions)
  }
})
onBeforeUnmount(() => {
  window.removeEventListener('scroll', handleScroll, passiveScrollListenerOptions)
  if (scrollParent && !(scrollParent instanceof Window)) {
    scrollParent.removeEventListener('scroll', handleScroll, passiveScrollListenerOptions)
  }
})
</script>
