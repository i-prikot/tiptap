...
import { Card, CardBody, CardItemGroup } from '../primitives'
    ...
    ? emojis
        .filter((emoji) => {
          ...
          return (
            ...
            emoji.shortcodes.some((code: string) => code.toLowerCase().includes(normalized)) ||
            emoji.tags.some((tag: string) => tag.toLowerCase().includes(normalized))
          )
        })
        .slice(0, 100)
    : emojis.slice(0, 100)
