...
export const TURN_INTO_BLOCKS: TurnIntoBlock[] = [
  ...
  { type: 'heading', label: 'Heading 1', level: 1, isActive: editor => editor.isActive('heading', { level: 1 }) },
  { type: 'heading', label: 'Heading 2', level: 2, isActive: editor => editor.isActive('heading', { level: 2 }) },
  { type: 'heading', label: 'Heading 3', level: 3, isActive: editor => editor.isActive('heading', { level: 3 }) },
  { type: 'bulletList', label: 'Bulleted list', isActive: editor => editor.isActive('bulletList') },
  { type: 'orderedList', label: 'Numbered list', isActive: editor => editor.isActive('orderedList') },
...
export function canTurnInto(editor: Editor | null, blockTypes?: TurnIntoBlockType[]): boolean {
  ...
  if (selection instanceof NodeSelection) return allowed.includes(selection.node.type.name as TurnIntoBlockType)
...
export function filterTurnIntoBlocks(blockTypes?: TurnIntoBlockType[]): TurnIntoBlock[] {
  return blockTypes ? TURN_INTO_BLOCKS.filter(block => blockTypes.includes(block.type)) : TURN_INTO_BLOCKS
}

/** Активная (или первая) опция для подписи триггера. */
export function getActiveTurnIntoBlock(editor: Editor | null, blockTypes?: TurnIntoBlockType[]): TurnIntoBlock {
